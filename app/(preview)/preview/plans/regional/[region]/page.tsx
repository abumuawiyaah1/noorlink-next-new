...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!product) {
    return { title: "Regional eSIM (v2 Preview) | NoorLink" };
  }

  return {
    title: `${product.displayName} (v2 Preview) | NoorLink`,
    robots: { index: false, follow: false },
  };
}

export default async function PreviewRegionalPlansPage({ params }: PageProps) {
  const { region } = await params;
